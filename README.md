...
class IoTSystem:
    ...
    def read_all_sensors(self):
        ...
        self.mq5 = self.read_analog_channel(0)
        self.ky026 = self.read_analog_channel(1)
        self.gas_status = "HIGH" if self.mq5 > self.limits['gas'] else "LOW"
        self.flame_status = "HIGH" if self.ky026 > self.limits['flame'] else "LOW"
        
        self.sw420_adc, self.sw420 = self.read_digital_channel(2, self.limits['vibr'])
        self.fc51_adc, self.fc51 = self.read_digital_channel(3, self.limits['obst'])
        self.float_level_adc, self.float_level = self.read_digital_channel(4, self.limits['level'])
        self.level = "HIGH" if self.float_level == 1 else "LOW"
        ...
        print(f"Gas: ADC={self.mq5:.1f}, Status={self.gas_status}")
        print(f"Flame: ADC={self.ky026:.1f}, Status={self.flame_status}")
        print(f"Vibration: ADC={self.sw420_adc:.1f}, Digital={self.sw420}")
        print(f"FC51 Obstacle: ADC={self.fc51_adc:.1f}, Digital={self.fc51}")
        print(f"Float Level: ADC={self.float_level_adc:.1f}, Digital={self.float_level}")
        
        return self.temp, self.hum, self.voltage, self.mq5, self.fc51, self.ky026, self.sw420, self.gas_status, self.flame_status, self.level, self.float_level, self.sw420_adc, self.fc51_adc, self.float_level_adc
    ...
    def receive_from_thingspeak(self, start_date=None, end_date=None):
        ...
        try:
            ...
            if self.system_active:
                ...
                self.lcd.clear()
                self.lcd.move_to(0, 1)
                self.lcd.putstr("DATA RECEIVED")
                self.lcd.move_to(0, 2)
                self.lcd.putstr(f"FILE: {filename}")
                time.sleep(0.1)
            else:
                self.lcd.clear()
                self.lcd.move_to(0, 1)
    ...
    def display_sensor_data(self):
        if self.lcd:
            self.lcd.clear()
            self.lcd.move_to(0, 0)
            self.lcd.putstr(f"TEMP:{self.temp if self.temp is not None else 'N/A'}C")
            self.lcd.move_to(0, 1)
            self.lcd.putstr(f"HUM:{self.hum if self.hum is not None else 'N/A'}%")
            self.lcd.move_to(0, 2)
            self.lcd.putstr(f"VOLTAGE:{self.voltage:.2f}V")
            self.lcd.move_to(0, 3)
            self.lcd.putstr(f"GAZ:{self.gas_status}")
            time.sleep(5)
            if not self.system_active:
                return
            self.lcd.clear()
            self.lcd.move_to(0, 0)
            self.lcd.putstr(f"FLAME:{self.flame_status}")
            self.lcd.move_to(0, 1)
            self.lcd.putstr(f"OBSTACLE:{self.fc51}")
            self.lcd.move_to(0, 2)
            self.lcd.putstr(f"VIBRATION:{self.sw420}")
            self.lcd.move_to(0, 3)
            self.lcd.putstr(f"LEVEL:{self.level}")
            time.sleep(5)
            self.lcd.clear()
    ...
    def display_sensor_limits(self):
        if self.lcd:
            ...
            self.lcd.move_to(0, 2)
            self.lcd.putstr(f"VOLT: {self.limits['voltage']}V")
            self.lcd.move_to(0, 3)
    ...
    def set_limits(self):
        ...
        if not self.system_active:
            return
        sensor_map = {'1': 'temp', '2': 'hum', '3': 'voltage', '4': 'gas', '5': 'flame', '6': 'vibr', '7': 'obst', '8': 'level'}
    ...
    def check_limits_and_alert(self):
        ...
        if self.hum is not None and self.hum > self.limits['hum']:
            ...
            simple_alert = True
        if self.voltage > self.limits['voltage']:
    ...
    def send_file_to_pc(self, ip_address, filename):
        ...
        try:
            ...
            if self.system_active:
                ...
                self.lcd.clear()
                self.lcd.move_to(0, 1)
                self.lcd.putstr("FILE SEND & DELETED")
            else:
                self.lcd.clear()
                self.lcd.move_to(0, 1)
                self.lcd.putstr("Operation Cancelled")
            ...
            self.lcd.clear()
            self.lcd.move_to(0, 1)
            self.lcd.putstr("Error Sending")
            self.lcd.move_to(0, 2)
            self.lcd.putstr("or Deleting")
        finally:
            conn.close()
            s.close()
            time.sleep(2)
